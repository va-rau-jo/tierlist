...
const TierListItemCard: React.FC<TierListItemCardProps> = ({ tierList, refreshCallback }) => {
	...
	return (
		<div
			className='relative bg-white/50 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden w-full'
			style={{ maxWidth: '90vw' }}
		>
			<Link
			...
			></Link>
			<div className='flex flex-col items-center px-4 py-2 sm:px-8 sm:py-2 cursor-pointer'>
				{tierList.isPrivate ? (
				...
				<h3
					className='text-xl md:text-3xl text-center font-bold text-gray-900 dark:text-white line-clamp-2'
					title={tierList.name}
				>
				...
				<section className='flex justify-around'>
					<div className='flex items-center mb-2 space-x-2 w-full'>
						...
						<div className='text-md md:text-lg text-nowrap'>
					...
					<div className='text-sm md:text-base flex my-auto w-full ml-8 py-1 justify-center space-x-3'>
						{canEdit && (
